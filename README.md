...
- **Личный канал уведомлений** (`ws/notifications/`): сообщение в любой из чатов приходит сюда, даже когда его чат не открыт, — отсюда бейдж непрочитанного в сайдбаре live. Сюда же летят `chat_read` (обнулить бейдж в других вкладках), `chat_deleted` и `member_removed` (убрать чат из списка)
...
- [ ] Удаление чата и переименование GROUP-чата из UI (бэкенд удаления готов)
