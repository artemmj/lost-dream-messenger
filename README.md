...
- Восстановление сессии при перезагрузке: профиль догружается через `/users/me/`
- Logout — клиентский (очистка токенов в localStorage)
- Редактирование своего профиля из UI (модалка «Профиль»): имя, фамилия, email, телефон — `PATCH /users/me/`. Телефон — это логин, поэтому после смены входить нужно новым номером (действующий JWT при этом остаётся валидным)
...
- Удаление чата из UI (кнопка в шапке, подтверждение вторым кликом): групповой — только админ, личного — любой участник; остальным чат убирается мгновенно (`chat.deleted` в личный канал + закрытие сокета чата кодом 4004)
- Переименование группового чата: название правится прямо в шапке окна (только админ) через `PATCH /chats/{id}/`; у остальных обновляется live — `chat_renamed` приходит в личный канал
- Список чатов с последним сообщением и собеседником
- **Счётчик непрочитанного** в списке чатов: курсор прочтения на участника (`Membership.last_read_at`), `unread_count` в REST, `POST /chats/{id}/read/` для отметки
...
- **Личный канал уведомлений** (`ws/notifications/`): сообщение в любой из чатов приходит сюда, даже когда его чат не открыт, — отсюда бейдж непрочитанного в сайдбаре live. Сюда же летят `chat_read` (обнулить бейдж в других вкладках), `chat_renamed` (обновить название чата), `chat_deleted` и `member_removed` (убрать чат из списка)
- **Онлайн-статус**: presence-реестр в Redis (учитывает несколько вкладок), снимок «кто онлайн» при подключении; в личном чате — точка у собеседника, в групповом — счётчик «в сети» в шапке и точки участников в панели; presence и `last_seen` ведёт именно личный канал, поэтому закрытие чата не «выключает» пользователя
...
- Sidebar со списком чатов, превью последнего сообщения и круглым бейджем непрочитанного (`99+` на больших числах)
- Окно чата: live-сообщения, presence собеседника, индикатор WS-соединения, панель участников группы
- Возврат вкладку в фокус = отметка прочтения открытого чата, поэтому счётчик не врёт после сна/потерянного соединения
